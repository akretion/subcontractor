...
class TestSubcontractorInvoice(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.subcontracted_product.sudo(cls.user_company_b.id).write({
            'property_account_income_id': cls.env.ref('account_invoice_inter_company.a_sale_company_b').id
        })

        # Create sale journal for company B as it was not done in account_invoice_inter_company module
        cls.env['account.journal'].create({
            'name': 'Sale journal - Company B',
            'code': 'SAJ-B',
            'type': 'sale',
            'sequence_id': cls.env.ref('account_invoice_inter_company.sequence_sale_journal_company_b').id,
            'update_posted': True,
            'company_id': cls.company_b.id,
        })

        # Create purchase journal for  company A as it was not done in account_invoice_inter_company module
        cls.env['account.journal'].create({
            'name': 'Purchases Journal - (Company A)',
            'code': 'EXJ-A',
            'type': 'purchase',
            'sequence_id': cls.env.ref('account_invoice_inter_company.sequence_purchase_journal_company_a').id,
            'update_posted': True,
        ...
        # may be change...)
        is_subcontractor = cls.env.ref('account_invoice_subcontractor.group_is_subcontractor')
    ...
    def test_customer_subcontractor(self):
        ...
        invoice = self.env['account.invoice'].create({
            ...
            'account_id': self.env.ref('account_invoice_inter_company.a_recv_company_a').id,
            'company_id': self.company_a.id,
        })
        invoice_line = self.env['account.invoice.line'].create({
            'product_id': self.subcontracted_product.id,
            'name': self.subcontracted_product.name,
            'account_id': self.env.ref('account_invoice_inter_company.a_sale_company_a').id,
            'price_unit': 200,
            'quantity': 2,
            'company_id': self.company_a.id,
        ...
        self.env['subcontractor.work'].sudo()._scheduler_action_subcontractor_invoice_create()
